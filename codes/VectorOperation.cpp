...
template<class T>vector<vector<T>> Scaled(const vector<vector<T>>& a, size_t k) {
	...
	for (size_t i = 0; i < h; ++i) {
		assert(a[i].size() == w);
		for (size_t j = 0; j < w; ++j) {
			for (size_t y = 0; y < k; ++y)for (size_t x = 0; x < k; ++x) {
...
template<class T>void Rotate90(vector<vector<T>>& a) { a = Rotated90(a); }
template<class T>void Rotate180(vector<vector<T>>& a) { a = Rotated180(a); }
template<class T>void Rotate270(vector<vector<T>>& a) { a = Rotated270(a); }
template<class T>void Scale(vector<vector<T>>& a, size_t k) { a = Scaled(a, k); }