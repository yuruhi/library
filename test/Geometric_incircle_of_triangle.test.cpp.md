...
data:
  ...
  bundledCode: "#line 1 \"test/Geometric_incircle_of_triangle.test.cpp\"\n#define\
    ...
    \ sgn(left_x() - r.left_x()) <= 0 && sgn(r.right_x() - right_x()) <= 0 &&\n\t\t\
    \t    sgn(top_y() - r.top_y()) <= 0 && sgn(r.bottom_y() - bottom_y()) <= 0;\n\t\
    \t}\n\t\tconstexpr bool contains(const Circle& c) const {\n\t\t\treturn sgn(left_x()\
    \ - c.left_x()) <= 0 && sgn(c.right_x() - right_x()) <= 0 &&\n\t\t\t    sgn(top_y()\
    \ - c.top_y()) <= 0 && sgn(c.bottom_y() - bottom_y()) <= 0;\n\t\t}\n\t\tfriend\
