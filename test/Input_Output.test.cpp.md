...
data:
  ...
  bundledCode: "#line 1 \"test/Input_Output.test.cpp\"\n#define PROBLEM \"https://judge.yosupo.jp/problem/many_aplusb\"\
    ...
    \ {\n\t\tput(v.first);\n\t\tput(D.d);\n\t\tput(v.second);\n\t}\n\ttemplate <class\
    \ InputIterater>\n\tvoid put_range(const InputIterater& begin, const InputIterater&\
    \ end) const {\n\t\tfor (InputIterater i = begin; i != end; ++i) {\n\t\t\tif (i\
    \ != begin) put(D.d);\n\t\t\tput(*i);\n\t\t}\n\t}\n\ttemplate <class T> void put(const\
    \ vector<T>& v) const {\n\t\tput_range(v.begin(), v.end());\n\t}\n\ttemplate <class\
    \ T, size_t N> void put(const array<T, N>& v) const {\n\t\tput_range(v.begin(),\
    \ v.end());\n\t}\n\ttemplate <class T> void put(const vector<vector<T>>& v) const\
    \ {\n\t\tfor (size_t i = 0; i < v.size(); ++i) {\n\t\t\tif (i) put(D.l);\n\t\t\
    \tput(v[i]);\n\t\t}\n\t}\n\n\tOutput() = default;\n\tOutput(const BoolStr& _boolstr,\
    \ const DivStr& _divstr) : B(_boolstr), D(_divstr) {}\n\tOutput& operator()()\
