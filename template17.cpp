...
struct Input {
	template<class T>operator T()const { return InputF<T>(); }
	int operator--(int)const { int v; InputF(v); v--; return v; }
	InputV operator[](int n)const { return InputV(n); }
	InputV operator[](const pair<int, int>& n)const { return InputV(n); }
	void operator()()const {}
	template<class H, class...T>void operator()(H&& h, T&& ...t)const { InputF(h); operator()(forward<T>(t)...); }
private:
