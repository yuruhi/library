...
data:
  ...
  attributes:
    links: []
  bundledCode: "#line 1 \"Utility/CountStraight.cpp\"\n// description : \u9023\u7D9A\
    \u3057\u3066 f(i), f(i, j) \u304C true \u3092\u8FD4\u3059\u500B\u6570\ntemplate\
    \ <class F> tuple<vector<int>, vector<int>> CountLR(int n, F f) {\n\tvector<int>\
    \ left(n);\n\tfor (int i = 0; i < n; ++i) {\n\t\tif (f(i)) left[i] = 1 + (i ?\
    \ left[i - 1] : 0);\n\t}\n\tvector<int> right(n);\n\tfor (int i = 0; i < n; ++i)\
  ...
  code: "// description : \u9023\u7D9A\u3057\u3066 f(i), f(i, j) \u304C true \u3092\
    \u8FD4\u3059\u500B\u6570\ntemplate <class F> tuple<vector<int>, vector<int>> CountLR(int\
    \ n, F f) {\n\tvector<int> left(n);\n\tfor (int i = 0; i < n; ++i) {\n\t\tif (f(i))\
    \ left[i] = 1 + (i ? left[i - 1] : 0);\n\t}\n\tvector<int> right(n);\n\tfor (int\
